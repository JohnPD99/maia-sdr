...
class TestCpwrPeak(AmaranthSim):
    ...
    def common_random_inputs(self, vcd=None):
        num_inputs = 10000
        re = np.array([random.randrange(-2**(self.width-1), 2**(self.width-1)-1)
                for _ in range(num_inputs)], dtype=object)

        im = np.array([random.randrange(-2**(self.width-1), 2**(self.width-1)-1)
                for _ in range(num_inputs)
        ], dtype=object)

        real = np.array([random.randrange(-2**(self.real_width-1), 2**(self.real_width-1)-1)
            for _ in range(num_inputs)
        ], dtype=object)
